 ...
 * @param radius Radius of circling animations
 */
function create(token, moteCount, {effect = undefined, id = 'Crown of Stars', file = 'jb2a.twinkling_stars.points07.white', scale = 0.5, radius = 0.5} = {} ) {
    ...
    function createStarMoteEffect(sequence, idx) {
        ...
        function createOrbit(sequence, objectName, delay) {
            return sequence.loopProperty(objectName, 'rotation', { from: 0, to: 360, duration: 5000, delay: delay });
        }

        sequence = createStar(sequence);
        sequence = createOrbit(sequence, 'sprite', 500);
        sequence = createOrbit(sequence, 'spriteContainer', 0);
        return sequence
